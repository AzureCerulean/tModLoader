...
tModLoader is an application programming interface (API) for Terraria which provides a way to create and load modifications (mods), without the need to directly use the Terraria source code.  This means that you can easily make mods compatible with other mods and save yourself the trouble of decompiling and recompiling the Terraria.exe.  This also means that players can use multiple mods simultaneously without having to worry about most compatibility issues.  Traditionally, most standalone mods without APIs (like tModLoader) may not be compatible with each other.  tModLoader is made to work for Terraria version 1.3 and above. 
To view our releases, you can visit the [releases page], (https://github.com/tModLoader/tModLoader/releases)  which contains the binary files needed to install tModLoader
